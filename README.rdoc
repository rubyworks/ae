...
test framework by checking for the existence of thier respective 
namespace modules.
...
With AE, defining asseretions centers around the use of the #assert method. So
"assert" is AE's primary nomenclature. However, variant nomenclatures have been
popularized by other test frameworks, terms like "should" and "must". If you
have grown acustom to these other terms or simply prefer one of them, AE
provides optional libraries for utilizing them.
...
and can be mixed into test environment, to provide old-school asseetion methods,
e.g.
