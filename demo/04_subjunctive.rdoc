...
With the antonym of <tt>should!</tt> (read "should not") or +shouldnt+, and for
+must+, <tt>must!</tt> and +wont+.
...
unlike +assert+ which simply checks to see that the argument evalutates
as true.
...
Addtional English forms are +a+ and +an+, equivalent to +be+ except
that they use <tt>case?</tt> instead of <tt>equate?</tt> when acting
